...
**הם מחפשים גלים. אנחנו מחפשים פלטה. פחות גלים. יותר ים.** אפליקציה בעברית לאוהבי ים שטוח: לשחייה, שנירקול, סאפ
ובטלה במים. עונה על השאלה האחת: האם עכשיו (או מחר) יש "ים פלטה". הגולשים יסתדרו.
...
אין חשבון משתמש. העדפות ומיקומים מותאמים נשמרים בדפדפן. בקשת מיקום נעשית רק בעקבות "קרוב אליי" או עיבוד תמונה;
אם אין חוף מוכר בקרבת מקום, המיקום שנשמר עשוי להתבסס על הקואורדינטות שלכם. קואורדינטות החוף נשלחות ל-Open-Meteo
ולשירות המצופים; פענוח קישור מפות שולח את הקישור ל-Cloudflare Worker שלנו ואת הקואורדינטות ל-Nominatim.

בהפעלת התראות נשמרים ב-Cloudflare KV: כתובת מנוי ה-Push, מפתחות המנוי, שם וקואורדינטות החוף ומצב ההתראות.
אין כיום תקופת מחיקה אוטומטית. כיבוי ההתראות מבטל את המנוי בדפדפן ומבקש למחוק את הרשומה בשרת.

תמונות מעובדות מקומית ב-canvas ואינן נשלחות לשרת של האפליקציה. שיתוף מתבצע ביוזמתכם דרך שירות השיתוף שבחרתם,
והתמונה עשויה לכלול שם חוף ושעה. GoatCounter מודד ביקורים ואירועי שימוש (בלי עוגיות ובלי מזהה אישי, אבל בקשת
הרשת עצמה מגיעה לספק). Google Fonts וספריית ה-QR נטענות מספקים חיצוניים; דיווח יזום על מצב הים נשלח ל-Google Forms
עם שם החוף ונתוני התחזית.
